...
def setup_agent_group():
    ...
    webpage_analyzer_llm = GroqLLM(model=LLMModel.LLAMA_3_1_70B_VERSATILE)
    ...
    webpage_analyzer_llm = MistralLLM(model=LLMModel.MISTRAL_LARGE) #GroqLLM(model=LLMModel.LLAMA_3_1_70B_VERSATILE)
    ...
    group.add_agent(google_search_agent)
    group.add_agent(webpage_analyzer_agent)
...
async def run_research_summary_workflow(agent_group):
    # Define the user task
    user_task = "find out the stock price of NVidia company"
